...
def agent_docs_block() -> str:
    return (
        ...
        "call `edgebase_goal` to get a Goal Capsule and Work Contract before editing. For multi-session work or "
        "handoffs, use `edgebase_checkpoint` after stable understanding, `edgebase_fork_plan` before trying an "
        "isolated branch plan, and `edgebase_resume` when continuing from local context. Do not wait for the user "
        "to request Edgebase explicitly. Context Branch records are local continuity state, not git commits.\n\n"
        ...
        "edgebase checkpoint \"<what you now understand>\"\n"
        "edgebase fork-plan \"<plan to try>\"\n"
        "edgebase resume\n"
        "```\n\n"
        ...
        "as project skills. Hooks and MCP calls may also update local graph artifacts under "
        "`.edgebase/graphs/latest.*`; use the surfaced artifact paths when a visual relationship view helps, "
        "but do not paste raw graph dumps into agent context. "
...
def claude_skill_content(repo_root: Path, command: str | None) -> str:
    ...
    return (
        ...
        "---\n"
        "name: edgebase\n"
        "description: Fetch Edgebase context or resume local Context Branch state for coding-agent work.\n"
        "argument-hint: \"<coding task | resume | checkpoint: message | fork-plan: message>\"\n"
        "---\n\n"
        "# Edgebase\n\n"
        "Fetch the current Edgebase context capsule for this task, or use Context Branches for cross-session "
        "continuity. If the response includes `.edgebase/graphs/latest.*` artifact paths, open or inspect those "
        "local files only when a visual relationship view helps the task.\n\n"
        "Prefer MCP tools when available:\n\n"
        "- `edgebase_context` before broad repository exploration or edits.\n"
        "- `edgebase_resume` when continuing previous local work.\n"
        "- `edgebase_checkpoint` after reaching a stable understanding or handoff point.\n"
        "- `edgebase_fork_plan` before trying an isolated branch plan.\n\n"
        "Fallback context command:\n\n"
        "```bash\n"
        f"{command_prefix} \"$ARGUMENTS\" --budget 1200\n"
        "```\n\n"
        "Fallback Context Branch commands:\n\n"
        "```bash\n"
        f"{shlex.join([executable, *prefix, '--root', str(repo_root), 'resume'])}\n"
        f"{shlex.join([executable, *prefix, '--root', str(repo_root), 'checkpoint'])} \"<message>\"\n"
        f"{shlex.join([executable, *prefix, '--root', str(repo_root), 'fork-plan'])} \"<message>\"\n"
        "```\n\n"
        "If `$ARGUMENTS` is empty, infer the task from the current user request.\n"
        f"{CLAUDE_SKILL_END}\n"
    )
...
def claude_goal_skill_content(repo_root: Path, command: str | None) -> str:
    ...
    return (
        ...
        "If the response includes `.edgebase/graphs/latest.*` artifact paths, use them as local visual aids "
        "without copying raw graph data into the coding context.\n\n"
        "```bash\n"
        f"{command_prefix} \"$ARGUMENTS\" --budget 1200\n"
        "```\n\n"
